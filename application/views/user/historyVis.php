...
<head>
	...
	<title>Historial Visitantes</title>
</head>
<body>
	<?= $navbar ?>
	<div class="containerG">
		...
		<?php endif; ?>
		<div class="btnElec">
			<button class="btnDesc"><a href="<?= base_url('user/historyVistT') ?>"><img src="<?= base_url('./assets/img/tableIcon.svg') ?>"></a></button>
			<button class="btnAct"><img src="<?= base_url('./assets/img/cardIcon.svg') ?>"></button>
		</div>
		<div class="space-search">
			<form class="formS" action="<?= base_url('user/searchVis') ?>" method="post"><input type="text" name="iden" placeholder="Buscar por identificación"><button><i class="bx bx-search nav__icon"></i></button></form>
		</div>
		<?php if ($data){ ?>
			...
			<div class="card">
				<div class="space1">
					...
					<div class="columns" >
						...
						<div class="column2">
							...
							<?php if(!$item->reason == null): ?><p class="p1">Motivo: <span><?= $item->reason ?></span></p><?php endif;?>
						</div>
					</div>
				...
				<div class="space2">
					...
					<?php } ?>
					<form action="<?= base_url('user/dropHisV')?>" method="post">
